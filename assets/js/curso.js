...
function environmentSection(course) {
  ...
  if (isInfoLab) {
    ...
    media = `<div class="course-environment-reference"><span aria-hidden="true">⌁</span><div><strong>Ambiente indicado: ${esc(ambiente)}</strong><p>O catálogo não possui foto específica deste laboratório. Consulte a página de ambientes para conhecer outros espaços técnicos da unidade.</p></div></div>`;
  } else {
...
if (!course) {
  document.body.innerHTML = '<main class="section"><h1>Curso não encontrado</h1><a href="../index.html">Voltar ao catálogo</a></main>';
} else {
  ...
  $("#courseTags").innerHTML = `${labels.badge(label)}<span class="tag turno">${esc(course.turno)}</span><span class="tag">${esc(course.area)}</span><span class="tag">${esc(course.nivel_exigencia)}</span>`;
  ...
  const video = xp.video ? `<article class="experience-section video-section"><div><span class="eyebrow">Vídeo complementar</span><h2>Entenda melhor esta área</h2><p>${esc(xp.video_title || 'Conheça a área e suas possibilidades')} — ${esc(xp.video_channel || 'YouTube')}.</p><p class="video-note">Conteúdo externo de apoio. O vídeo não substitui o edital e não representa material oficial do CETAM.</p></div><div class="video-frame"><iframe src="https://www.youtube-nocookie.com/embed/${encodeURIComponent(xp.video)}?rel=0" title="${esc(xp.video_title || course.nome_curso)}" loading="lazy" referrerpolicy="strict-origin-when-cross-origin" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe></div></article>` : '';
